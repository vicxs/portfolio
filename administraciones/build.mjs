...
// en → administraciones/en/index.html. Copy lives in strings.mjs; edit it
...
const pageUrl = (lang) => `${SITE}administraciones/${LANGS[lang].dir}`;
...
export function renderPage(lang) {
  ...
  // Relative path from this page back to the site root.
  const root = meta.dir ? '../../' : '../';
  const assets = `${root}assets/`;
  const own = `${root}administraciones/`;
  // The portfolio has English and Spanish; Valencian readers get Spanish.
...
<link rel="stylesheet" href="${root}portfolios/cabanyal.css?v=8" />
<link rel="stylesheet" href="${own}administraciones.css?v=3" />
</head>
<body>
  ...
  <div class="wrap">
    <div class="grid">
      ...
      <nav class="nav" aria-label="${esc(t.sectionsLabel)}">
        <a class="link" href="#servicios">${esc(t.navServices)}</a>
        <a class="link" href="#como-trabajo">${esc(t.navHow)}</a>
        <span class="lang" role="group" aria-label="${esc(t.languageLabel)}">${switcher}</span>
        <a class="btn btn-sm" href="#contacto">${esc(t.navDemo)}</a>
      </nav>
    </div>
  </div>
</header>
...
${cards}
    ...
    <aside class="taller" aria-labelledby="taller-titulo">
      ...
      <div class="taller-side">
        ...
        <a class="btn btn-light" href="${mailto(t.workshopSubject)}">${esc(t.workshopCta)}</a>
      </div>
    </aside>
  </section>
...
${principles}
  ...
  <div class="wrap pair">
    <section id="demos" class="panel" aria-labelledby="demos-titulo">
      ...
      -->
      <p>${esc(t.demosText)}</p>
      <a class="link link-on" href="${demo}">${esc(t.demosCta)}</a>
    </section>
    ...
    <section id="datos" class="panel" aria-labelledby="datos-titulo">
      ...
      <a class="link link-on" href="${mailto(t.dataSubject)}">${esc(t.dataCta)}</a>
    </section>
  </div>
  ...
  <section id="contacto" class="closing" aria-labelledby="contacto-titulo">
    ...
    <div class="wrap closing-inner">
      ...
      <div class="closing-contact">
        ...
        <p class="muted">${esc(t.closingPlace)} · <a class="link" href="${LINKEDIN}" target="_blank" rel="noreferrer">LinkedIn ↗</a></p>
        <a class="btn" href="${demo}">${esc(t.closingCta)}</a>
      </div>
    </div>
  </section>
</main>
...
export const outputPath = (lang) => path.join(here, LANGS[lang].dir, 'index.html');

if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  for (const lang of Object.keys(LANGS)) {
    const out = outputPath(lang);
    mkdirSync(path.dirname(out), { recursive: true });
    writeFileSync(out, renderPage(lang));
